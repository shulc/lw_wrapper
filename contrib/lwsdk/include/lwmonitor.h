...
#define LWMONITOR_GLOBAL    "LWM: Dynamic Monitor"

/* $wrapper: type=global, name=LWMonitor$ */
...
#define LWLMONFUNCS_GLOBAL        "LWLMonFuncs 2"
 ...
 * DELAYEDOPEN:
 *  Delays opening the monitor window to avoid the window from popping up
 *  for short progress periods.
 * PROGRESSONLY:
 *  Shows only the progress bar, not the history.
 ****
 */
...
#define LMO_TOOLBAR         (1<<4)
#define LMO_DELAYEDOPEN     (1<<5)
#define LMO_PROGRESSONLY    (1<<6)

/* TypeDefs */
typedef struct st_LMONDATA *LWLMonID;

/* $wrapper: type=global, name=LWLMonFuncs$ */
typedef struct st_LWLMonFuncs {
  LWLMonID   (*create)    ( void );
  void       (*setup)     ( LWLMonID mon, LWCStringUTF8 title, unsigned int uiflags, LWCStringUTF8 histfile );
  void       (*setwinpos) ( LWLMonID mon, int x, int y, int w, int h );
  void       (*init)      ( LWLMonID mon, unsigned int total, LWCStringUTF8 msg );
  int        (*step)      ( LWLMonID mon, unsigned int incr, LWCStringUTF8 msg );
...
#define LMON_DFLT(fun,mon,tot)     {(*fun->setup)(mon,"Processing...",0,NULL);(*fun->init)(mon,tot,NULL);}
