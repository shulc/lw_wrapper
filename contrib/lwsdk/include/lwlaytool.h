 ...
 *
 * end      process a mouse-up event.  This is only called for handle
 *      processing (the down function returned false).
 */
/* $wrapper: type=base, name=LWLayoutToolFuncs, prefix=tool$ */
typedef struct st_LWLayoutToolFuncs {
    ...
    /* $return: method=ILayoutToolFuncs:start, value=-1$ */
    int         (*start)  (LWInstance, LWToolEvent *);
    /* $return: method=ILayoutToolFuncs:adjust, value=-1$ */
...
/* $wrapper: type=handler, name=LWLayoutTool, accessor=LWLAYOUTTOOL_CLASS, activate=oneshot$ */
