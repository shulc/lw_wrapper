...
#define LWDOPETRACK_GLOBAL      "Dopetrack Proxy 2"
...
typedef LWCStringUTF8   (*DTMenuItem)(int,int);
...
typedef struct _dtdrawcallbacks
{
    ...
    void        (*text)(int x,int y,int color,LWCStringUTF8 text);
    void        (*text_box)(int x,int y,int w,int h,LWCStringUTF8 text);
...
typedef struct st_LWDopetrackProxy
{
    ...
    void            (*exposeEnvelopes)(LWEnvelopeID *,LWCStringUTF8 *channel_names,int *states);
