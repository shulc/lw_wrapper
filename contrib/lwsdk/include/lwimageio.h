...
#define LWIMAGELOADER_VERSION 4

#define LWIMAGESAVER_CLASS "ImageSaver"
#define LWIMAGESAVER_VERSION 4
...
typedef struct st_LWPixelBGRA32 {
    unsigned char    b;
    unsigned char    g;
    unsigned char    r;
    unsigned char    a;
...
typedef struct st_LWPixelRGBINT {
    int              r;
    int              g;
    int              b;
} LWPixelRGBINT;

typedef struct st_LWPixelRGBAINT {
    int              r;
    int              g;
    int              b;
    int              a;
...
typedef enum en_LWImageParam {
    ...
    LWIMPAR_COMPUTERNAME,               /* Computer name. (char *)                  */
    LWIMPAR_LIGHTWAVESCENE,             /* Scene file name. (char *)                */
    LWIMPAR_CAMERAHFOV,                 /* Camer Horizontal FOV (float)             */
    LWIMPAR_CAMERAVFOV,                 /* Camer Vertical   FOV (float)             */
    LWIMPAR_SIZEOF
} LWImageParam;
...
typedef enum en_LWMakerNote {
    ...
    LWMN_ASPECT,                        /* x / y Pixel Aspect.   */
    LWMN_CAMERATRANSFORM,               /* Camera matrix.        */
    LWMN_BUILDNUMBER,                   /* LightWave Build Number*/
    LWMN_STEREO360,                     /* TRUE if Stereo 360    */
    LWMN_UDIM,                          /* UDim image.           */
    LWMN_DIFFUSEBOUNCES,                /* Diffuse Bounces.      */
    LWMN_RAYLIMIT,                      /* Ray Recursion Limit.  */
    LWMN_TRANSPARENCYLIMIT,             /* Transparency Recursion Limit. */
    LWMN_REFLECTIONLIMIT,               /* Reflection Recursion Limit.   */
    LWMN_REFRACTIONLIMIT,               /* Refraction Recursion Limit.   */
    LWMN_RAYPRECISION,                  /* Ray Precision.        */
    LWMN_REFLECTIONSAMPLES,             /* Reflection Samples.   */
    LWMN_REFRACTIONSAMPLES,             /* Refraction Samples.   */
    LWMN_SCATTERINGSAMPLES,             /* Subsurface Scattering Samples.*/
    LWMN_SIZEOF
} LWMakerNote;

/* $comment: ------------------------------------------------------------------- */
/* $comment: All data members of LWImageProtocol need to be const'd so that      */
/* $comment: destruction updating is disabled.  The access pointer returned      */
/* $comment: by LWImageLoaderLocal::begin() is out of scope by that point,       */
/* $comment: and the internal pointer is dangling, so it should not be accessed. */
/* $comment: ------------------------------------------------------------------- */
/* $wrapper: type=access, name=LWImageProtocol$ */
typedef struct st_LWImageProtocol {
    /* $modifier: class=LWImageProtocol, arg=type, modifier=const$ */
    int       type;
    /* $modifier: class=LWImageProtocol, arg=priv_data, modifier=const, apply=pointer$ */
    void     *priv_data;
    /* $comment: ------------------------------------------------------------------ */
    /* $comment: These next two $extend lines extract the function prototypes from  */
    /* $comment: the LWImageProtocol structure and add them to the class definition */
    /* $comment: IImageSaver generated by the wrapper.  The 'args' parameter causes */
    /* $comment: the first argument of each ("void *") to be discarded in the copy. */
    /* $comment: ------------------------------------------------------------------ */
    /* $extend: class=LWImageProtocol, args=fffe, group=IImageSaver, extract=done, extract=setSize, extract=setParam$ */
    /* $extend: class=LWImageProtocol, args=fffe, group=IImageSaver, extract=setMap$ */
    int     (*done    )( void *, int );
    void    (*setSize )( void *, int, int );
    int     (*setParam)( void *, LWImageParam parameter, const char *value_bytes /* encoded? (when parameter uses string) */ );
    int     (*sendLine)( void *, int, const LWPixelID );
    void    (*setMap  )( void *, int, const unsigned char rgb[ 3 ] );
} LWImageProtocol;
/* $comment: This typedef is stand-alone so the wrapper system is not confused. */
typedef LWImageProtocol * LWImageProtocolID;
...
/* $wrapper: type=access, name=LWImageLoaderLocal$ */
...
/* $wrapper: type=access, name=LWImageSaverLocal$ */
typedef struct st_LWImageSaverLocal {
    /* $extend: class=LWImageSaverLocal, signature="int flags"$ */
    void               *priv_data;
    int                 result;
    LWImageType         type;
    LWCStringUTF8       filename;
    LWMonitor          *monitor;
    /* $wrapper: type=ignore, name=LWImageSaverLocal, arg=sendData$ */
