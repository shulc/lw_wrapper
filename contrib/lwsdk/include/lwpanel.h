 ...
 * $copyright$
 */
#ifndef LWSDK_LWPANEL_H
#define LWSDK_LWPANEL_H

#include <lwtypes.h>
#include <lwserver.h>
#include <lwxpanel.h>

#define PANEL_SERVICES_NAME     "LWPanelServices 3"
#define LWPANELFUNCS_GLOBAL     PANEL_SERVICES_NAME
#define LWRASTERFUNCS_GLOBAL    "RasterServices 4"
#define LWCONTEXTMENU_GLOBAL    "ContextMenuServices 2"
#define LWPANELMENU_GLOBAL      "PanelMenuServices 2"
#define LWPANELS_API_VERSION    23
...
#define LWT_MULTILINE    24
#define LWT__LAST        LWT_MULTILINE

typedef struct st_LWValString {
    LWType               type;
    LWMutableCStringUTF8 buf;
    int                  bufLen;
} LWValString;
...
typedef struct st_LWPanEditorDesc {
    LWType               type;
    LWMutableCStringUTF8 buf;
    int                  bufLen;
    int                  width;
...
typedef struct st_LWPanChoiceDesc {
    LWType          type;
    LWCStringUTF8  *items; /* NULL-terminated */
    int             vertical;
} LWPanChoiceDesc;

typedef struct st_LWPanTextDesc {
    LWType          type;
    LWCStringUTF8  *text /* NULL-terminated */;
} LWPanTextDesc;
...
/* $wrapper: type=ignore, name=LWPanLWItemDesc$ */
typedef struct st_LWPanLWItemDesc {
    LWType                type;
    void                 *global;
    int                   itemType;
    LWMutableCStringUTF8 *list /* NULL-terminated */;
    int                   count;
    int                   width;
} LWPanLWItemDesc;

/* $wrapper: type=ignore, name=LWPanPopupDesc$ */
typedef struct st_LWPanPopupDesc {
    LWType            type;
    int               width;
    int             (*countFn)(void *);
    LWCStringUTF8   (*nameFn)(void *, int);
    LWCStringUTF8   (*faceFn)(void *, int);
    int             (*typeFn)(void *, int);
    void           *(*subMFn)(void *, int);
} LWPanPopupDesc;

/* $wrapper: type=ignore, name=LWPanListBoxDesc$ */
...
/* $wrapper: type=ignore, name=LWPanMultiListBoxDesc$ */
typedef struct st_LWPanMultiListBoxDesc {
    LWType          type;
    int             width, visItems, top;  // width in pixels, height in items, top visible item
    int           (*countFn)(void *);
    LWCStringUTF8 (*nameFn)(void *, int item, int column);
    // pixel width of column i, terminate with 0
    // -1: use whatever remains
    // -2: use content to determine width
    // -3: use header to determine width
    int             (*colWidth)(void *, int i);
} LWPanMultiListBoxDesc;

/* $wrapper: type=ignore, name=LWPanXPanDesc$ */
...
/* $wrapper: type=ignore, name=LWPanTreeDesc$ */
typedef struct st_LWPanTreeDesc {
    ...
    LWCStringUTF8 (*infoFn)(void *data, void *node, int *nodeFlags); /* return "name" for node, fill 0 flags, or save non-zero flags */
...
/* $wrapper: type=ignore, name=LWPanMultiTreeDesc$ */
typedef struct st_LWPanMultiTreeDesc {
    LWType          type;
    int             width, height;
    int           (*countFn)(void *data, void *node); /* return number of leafs under given node, NULL node is root */
    void         *(*leafFn)(void *data, void *node, int i); /* return leaf i of node, NULL node is root */
    LWCStringUTF8 (*infoFn)(void *data, void *node, int column, int *nodeFlags); /* return "name" for node, fill 0 flags, or save non-zero flags */
    void          (*moveFn)(void *data, void *node, void *parentNode, int i); /* NULL prevents moves, or called when user moves node, with new parent, position */
    int           (*colWidth)(void *, int i);  /* pixel width of column i, terminate with 0; -1 to share remaining available width; -4096 to indicate next width is actually the column idx to use as the hierarchy column */
} LWPanMultiTreeDesc;
...
typedef enum {           /* Tags to get/set panel attributes */
    ...
    CTL_COLORSPACE,             /* color space for color square.       */
} cTag;
...
typedef union un_LWPanControlDesc {
    ...
    LWPanEditorDesc         editor;
} LWPanControlDesc;

typedef LWPanControlDesc                ControlDesc;
/* $wrapper: type=access, name=DrawFuncs$ */
typedef struct st_DrawFuncs {
    ...
    int     (*textWidth)(LWPanelID,LWCStringUTF8 text);             /* text */
    void    (*drawText)(LWPanelID,LWCStringUTF8 text,int,int,int);  /* text,color,x,y */
    const LWDisplayMetrics   *(*dispMetrics)();
} DrawFuncs;

/* $wrapper: type=ignore, name=LWControl$ */
...
/* $wrapper: type=ignore, name=LWPanelFuncs$ */
typedef struct st_LWPanelFuncs {
    LWPanelID               (*create)(LWCStringUTF8 title, void *);
    ...
    LWControl              *(*addControl)(LWPanelID pan, LWCStringASCII type, ControlDesc *data, LWCStringUTF8 label);
    ...
    void                    (*setParent)(LWPanelID pane, LWPanelID parent);     /* call BEFORE using open() */
    LWRasterID              (*getRaster)(LWPanelID pane); /* access the raster for the panel */
} LWPanelFuncs;

/* $wrapper: type=global, name=LWRasterFuncs$ */
typedef struct st_LWRasterFuncs {
    ...
    void            (*drawText)( LWRasterID raster, LWCStringUTF8 text, int color, int x, int y);
    ...
    int             (*textWidth)( LWRasterID raster, LWMutableCStringUTF8 text);

    /**
     * draw the given rich text into a given box, clipping as needed.
     * Rich text uses control codes to tag the text with meta data.
     */
    void            (*drawRichText)( LWRasterID raster, LWCStringUTF8 text, int color, int x, int y, int w, int h);

    /* determine the dimensions required for the given rich text */
    /* $argument: method=LWRasterFuncs:getRichTextSize, arg3=out:int:, arg4=out:int:$ */
    void            (*getRichTextSize)( LWRasterID raster, LWCStringUTF8 text, int *w, int *h);

    /* draw a portion of an image to a portion of a raster, scaling as needed. */
    /* Flags are unused, atm, set to 0 */
    void            (*drawImage)(LWRasterID raster, LWPixmapID image, int sx, int sy, int sw, int sh, int dx, int dy, int dw, int dh, int flags);

    /* transfer the raster to an existing image. No scaling is performed. */
    void            (*transferToImage)(LWRasterID raster, LWPixmapID image);

    /* draw a filled polygon using an array of x and y coordinates. */
    /* fractional coordinates allow for the edges of pixels to be specified. */
    void            (*fillPolygon)(LWRasterID raster, int color, float *x, float *y, int coord_count);

    /* draw a polygon outline using an array of x and y coordinates. */
    /* fractional coordinates allow for the edges of pixels to be specified. */
    /* pen_width determines the line size (0.0 for smallest) */
    void            (*strokePolygon)(LWRasterID raster, int color, float *x, float *y, int coord_count, float pen_width);

    /* restricts the area for subssequent drawing to an intersection of the current clipping and the provided rectangle*/
    void            (*pushClippingRect)(LWRasterID raster, int cx, int cy, int cw, int ch);

    /* removes the most recently pushed clipping rectangle, restoring the previous clipping area */
    void            (*popClippingRect)(LWRasterID raster);

    /* queries the current clipping area; returns number of pushed clipping rects (0 if no clipping exists) */
    /* The clipping area may have 0 width and/or 0 height */
    int             (*getClippingRect)(LWRasterID raster, int *ix, int *iy, int *iw, int *ih);

    /* queries the backing store scalar for the given raster. */
    /* All rasters measure coordinates in a common unit, regardless of what DPI display is used. */
    /* The backing scalar is relative to a 1 pixel to 1 unit (1:1) factor. */
    /* A factor of 2.0 means the backing memory for the raster is using 2 pixels for each unit. */
    /* macOS systems may offer whole numbers such as 1.0, 2.0, 4.0, while Windows often can offer fractional scalars like 1.25, 1.75, 3.5. */
    float           (*getBackingScalar)(LWRasterID raster);

} LWRasterFuncs;

/* $wrapper: type=global, name=ContextMenuFuncs, accessor=LWCONTEXTMENU_GLOBAL$ */
...
#define CTLF_INVISIBLE          0x200 // (read/write) ... erased
#define CTLF_GHOST              CTLF_DISABLE
#define CTLF_DPIAWARE           0x400 // (read/write) aware of dpi differences from a base dpi (only "OpenGLControl" can be changed and initial setting may differ from other controls depending on OS version)
...
/* Popup extra type return codes. */

#define LWPEXTYPE_NONE          0
#define LWPEXTYPE_STATE         0x8000
#define LWPEXTYPE_STATE_SEL     0x4000
#define LWPEXTYPE_STATE_MIX     0x2000
#define LWPEXTYPE_UNSELECTED    (LWPEXTYPE_STATE)
#define LWPEXTYPE_SELECTED      (LWPEXTYPE_STATE | LWPEXTYPE_STATE_SEL)
#define LWPEXTYPE_MIXED         (LWPEXTYPE_STATE_MIX)
#define LWPEXTYPE_HIGHLIGHT     0x0200
#define LWPEXTYPE_DISABLED      0x0100
#define LWPEXTYPE_SEP_BITS      0x000F
...
#define COLOR_CTL(pfunc,pan,tit)    (desc.type=LWT_VFLOAT,\
                                    (*pfunc->addControl) (pan, "ColorControl", &desc, tit))
...
#define CUSTPOPUPEXTRA_CTL(pfunc,pan,tit,w,name,num,face,typef,subm)(desc.type=LWT_POPUP,desc.popup.width=w,desc.popup.nameFn=name,\
                                    desc.popup.countFn=num,desc.popup.faceFn=face,\
                                    desc.popup.typeFn=typef,desc.popup.subMFn=subm,\
                                    (*pfunc->addControl)(pan, "CustomPopupControlExtra", &desc, tit))
...
#define FLOATSLIDER_CTL(pfunc,pan,tit)    (desc.type=LWT_RANGE,\
                                    desc.range.min=0,desc.range.max=100,\
                                    (*pfunc->addControl)(pan, "MiniFloatControl", &desc, tit))

#define INTSLIDER_CTL(pfunc,pan,tit)    (desc.type=LWT_RANGE,\
                                    desc.range.min=0,desc.range.max=100,\
                                    (*pfunc->addControl)(pan, "MiniSliderControl", &desc, tit))
...
#define MULTILINE_CTL(pfunc,pan,tit,w,h) (desc.type=LWT_MULTILINE,desc.editor.width=w,desc.editor.height=h,\
                                    (*pfunc->addControl)(pan, "MultilineControl", &desc, tit))
...
#define PAN_GETQUALIFIERS(pfunc,pan) ((*pfunc->get)(pan,PAN_QUALIFIERS,(void *)&(ival.intv.value)),ival.intv.value)
