...
#define LWSCENEEDITOR_GLOBAL "SceneEditorGlobal 2"
...
#define SECOLTYPE_FILE      LWID_( '_', 'F', 'I', 'L' )  /*  char *    -  Text string representing a file (PFX File)                                                      */ /* unicode */
#define SECOLTYPE_STRING    LWID_( '_', 'S', 'T', 'R' )  /*  char *    -  Text string (Item Name, Comment)                                                                */ /* unicode */
...
#define SECOLTYPE_VMAP      LWID_( '_', 'V', 'M', 'P' )  /*  SEVMap * -  Name of a VMAP (e.g. Bone Weight Map).
    ...
    */
#define SECOLTYPE_TEXTURE   LWID_( '_', 'T', 'X', 'R' )  /*  LWTextureID -  ID of a texture */
...
typedef enum en_SEBaseTypes {
  ...
  SEBASETYPE_SCENE, /* item is name of scene (LWCStringUTF8) (not used)*/
  SEBASETYPE_ITEM,  /* item is a LWItemID */
  SEBASETYPE_SURFACEGROUP, /* item is name of group (LWCStringUTF8) */
...
typedef struct st_SEColumn {
  ...
  unsigned int         (*apply)    ( LWTime time, unsigned int numitems, SEApply *items, void (*progress_increment)( void)); 
...
/* choice list construction */
  ...
  LWCStringUTF8        (*listName)    ( void *item, int index );
  ...
  int                  (*customDraw)  ( void *value, LWRasterID raster, int left, int top, unsigned int width, unsigned int height, unsigned int flags);
...
{
  LWID           id;                                               /* Bank ID.  Use the LWID_() macro to convert a string into an ID.                          */
  LWCStringUTF8  category;                                         /* name of category this bank belongs to.  can be NULL.  Banks with same category name are grouped together */
  LWCStringUTF8  title;                                            /* Bank Title, for display purposes.  Used in the Bank Popup. Also used to organize column selection */
...
/*  int          (*numRows      ) ( unsigned int bank_id, void *id);*//* Num Rows Function.  Scene Editor calls this to ask how many rows this bank needs for this item ID */
  void         (*beginProcess ) ( LWID bank_id);                   /* Called before Scene Editor calls any query/evaluate functions.  Use this to get bank-specific globals, init arrays, etc. */
  void         (*endProcess   ) ( LWID bank_id);                   /* Called after Scene Editor has called all the query/evalute functions.  Use this to free whatever you got during beginProcess() */
} SEBank;
...
typedef struct st_SceneEditorGlobal
{
    ...
    int (*unregisterBank)         ( LWID bank_id);                 /* Unregister an existing bank from the Scene Editor.                                         */
    int (*registerBankMaster)     ( LWCStringUTF8 master_name);    /* Register a new bank master with Scene Editor.  This plug-in will be launched automatically when the scene Editor is first activated.  Returns 1 on succes, -1 if already added and 0 on error */
    int (*unregisterBankMaster)   ( LWCStringUTF8 master_name);    /* Unregister a bank master.  Returns 1 on succes, -1 if already added and 0 on error                                                                                */
    unsigned int (*numInstances)  ( void);                         /* return number of instances of the Scene Editor running */
    int (*command)                ( LWCStringUTF8 command_string, void *command_return_data);   /* Issue a command to the Scene Editor global service.  Returns 0 on error */
