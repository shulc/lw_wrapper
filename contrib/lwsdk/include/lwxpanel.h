...
#define LWXPANELFUNCS_GLOBAL "LWXPanel 3"

/* Hint Tags */

typedef void *                  LWXPanelHint;

#define XpH(x)                  ((void*)(size_t)(x))
...
typedef struct st_LWXPDrawFuncs {
        ...
        void   (*drawRGBPixel) ( LWXPDrAreaID p, int r, int g, int b, int x, int y );
        void   (*drawLine)     ( LWXPDrAreaID p, int c, int x, int y, int x2, int y2 );
        void   (*drawBox)      ( LWXPDrAreaID p, int c, int x, int y, int w, int h );
        void   (*drawRGBBox)   ( LWXPDrAreaID p, int r, int g, int b, int x, int y, int w, int h );
        void   (*drawBorder)   ( LWXPDrAreaID p, int indent, int x, int y, int w, int h );
        int    (*textWidth)    ( LWXPDrAreaID p, LWCStringUTF8 s );
        int    (*textHeight)   ( LWXPDrAreaID p, LWCStringUTF8 s );
        void   (*drawText)     ( LWXPDrAreaID p, LWCStringUTF8 s, int c, int x, int y );
} LWXPDrawFuncs;
 ...
 * CLOSED    - The opened panel has closed and is no longer visible by the user
 */
...
/* Event notification */
typedef LWXPRefreshCode LWXPanelChangeNotifyFunc( LWXPanelID pan, unsigned int cid, unsigned int vid, int event_type );

/* Button click event notification */
typedef void  LWXPanelBtnClickFunc( LWXPanelID pan, int cid );

/* Popup command */
typedef void  LWXPanelPopCmdFunc( LWXPanelID pan, int cid, int cmdid );

/* Popup choice */
typedef int           LWXPanelPopCntFunc  ( void *userdata );
typedef LWCStringUTF8 LWXPanelPopNameFunc ( void *userdata, int idx );

/* Text transform function */
typedef LWCStringUTF8 LWXPanelTextXFormFunc ( LWXPanelID pan, unsigned int cid, LWCStringUTF8 string, LWXPanelTextXFormMode mode );

/* Draw Function */
typedef void LWXPanelControlDrawFunc(LWXPanelID pan, unsigned int cid, LWXPDrAreaID reg, int w, int h);
typedef void LWXPanelControlZoomFunc(LWXPanelID pan, unsigned int cid, int x, int y, int *region, int clickcount);
...
{
    LWCStringUTF8 label;    // utf-8 rich text for header (if header is shown)
...
{
    int button; // pointer button press/release 1-left, 2-middle, 3-right
    int count; // number of left-button pointer clicks; 0 for release
    int qualifiers; // IQ_ defines
} LWXPanelSheetEvent, *LWXPanelSheetEventRef;
...
//#define XPANELSHEET_SELECT_MODE_REPLACE  0
//#define XPANELSHEET_SELECT_MODE_ADD      1
//#define XPANELSHEET_SELECT_MODE_REMOVE   2
//#define XPANELSHEET_SELECT_MODE_TOGGLE   3
...
typedef struct st_LWXPanelSheetFuncs
{
    ...
    LWCStringUTF8 /* rich text */ (*getText)(LWXPanelID pan, unsigned int cid, LWXPanelSheetItemRef item, int column_index); // 1+
...
/* Ranch */
/* A ranch is a group (optional border visible) that links to a global ranch
 * registry.  The ranch can reference a Xpanel to present within it.
 * (clipping is not enforced)
 * The size is either static or dynamic.
 * When dynamic, the size comes from the referenced panel size (if one exists).
 * The control value is a pointer to the Xpanel ID (LWXPanelID) to present
 */
/* (unsigned int) bitwise feature flags */
#define XPANELRANCH_FEATURE_BORDER         (1<<0) /* show a border */
#define XPANELRANCH_FEATURE_AUTOWIDTH      (1<<1) /* use ideal width of ranched pane */
#define XPANELRANCH_FEATURE_AUTOHEIGHT     (1<<2) /* use ideal height of ranched pane */
#define XPANELRANCH_FEATURE_ALLOWPANEL     (1<<5) /* allow a separate panel to present the pane when the ranch is too small */

#define XPTAG_RANCH_FEATURE XpHTAG(382,2,0)
#define XpRANCHFEATURE(cid,on) XPTAG_RANCH_FEATURE, XpH(cid), XpH(on)

#define XPTAG_RANCH_AUTOWIDTHMAX XpHTAG(383,2,0)
#define XpRANCHAUTOWIDTHMAX(cid,i) XPTAG_RANCH_AUTOWIDTHMAX, XpH(cid), XpH(i)

#define XPTAG_RANCH_AUTOHEIGHTMAX XpHTAG(384,2,0)
#define XpRANCHAUTOHEIGHTMAX(cid,i) XPTAG_RANCH_AUTOHEIGHTMAX, XpH(cid), XpH(i)

#define XPTAG_RANCH_PANELCONFIGID XpHTAG(385,2,0)
#define XpRANCHPANELCONFIGID(cid,i) XPTAG_RANCH_PANELCONFIGID, XpH(cid), XpH(i)
...
typedef struct st_LWXPanelFuncs {
    ...
    void            (*describe)     ( LWXPanelID panel, LWXPanelDataDesc *dataDesc, LWXPanelGetFunc *get, LWXPanelSetFunc *set );
    void            (*hint)         ( LWXPanelID panel, unsigned int an_id, LWXPanelHint *hints );
    void            *(*getData)     ( LWXPanelID panel, unsigned int data_id );
    void            (*setData)      ( LWXPanelID panel, unsigned int data_id, void *data );
    void            *(*formGet)     ( LWXPanelID panel, unsigned int vid );
    void            (*formSet)      ( LWXPanelID panel, unsigned int vid, void *value );
