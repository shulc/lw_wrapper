...
typedef struct st_LWWireDrawAccess
{
    ...
    void (*lineTo)(void *data, LWFVector pos, int coord_system);
    ...
    void (*circle)(void *data, double radius, int);
...
typedef struct st_LWToolFuncs
{
    ...
    /***********************************************/
    ///Allow drawing to the UV interface
    //display a wireframe representation of the tool in the UV viewport.
    void(*drawUV)     (LWInstance, LWWireDrawAccess *);

    /// Gets called when the UV scene needs to be drawn.
    /// @return true if the tool successfully drew the scene, or false to 
    /// let the system handle it.
    int(*drawSceneUV)(LWInstance, int viewport_index);

    /// Draws an overlay to the UV viewport. OpenGL calls can be used here.
    void(*drawOverlayUV)(LWInstance, LWWireDrawAccess* drawer, int viewport_index, int pass);
} LWToolFuncs;
