 ...
 */
typedef void *  GlobalFunc (LWCStringASCII serviceName, int useMode);
...
#define SRVTAG_UNIQUE       0xA0000
#define SRVTAG_REPLACES     0xB0000
#define SRVTAG_EXCLUDE      0xC0000

typedef struct st_ServerTagInfo
{
    LWCStringASCII   string;
    unsigned int     tag;
...
#define LWSERVERINFO_GLOBAL "LW Server Info 2"

/* $wrapper: type=global, name=LWServerInfo$ */
typedef struct st_LWServerInfo
{
    LWCStringUTF8   (*path)  (LWCStringASCII server_class, LWCStringUTF8 server_name );
    LWCStringASCII *(*list)  (LWCStringASCII server_class);
} LWServerInfo;
