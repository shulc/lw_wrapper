...
#define LWPSYSFUNCS_GLOBAL  "Particle Services 3"
/*
...
#define LWPSB_RAGE      (1<<15)/* relative age: to provide information about the life expectancy */
#define LWPSB_POS0      (1<<16)/* start position (float[3]) */
#define LWPSB_SCL0      (1<<17)/* start scale (float[3]) */
#define LWPSB_ROT0      (1<<18)/* start rotation (float[3]) */
...
/* $wrapper: type=global, name=LWPSysFuncs$ */
typedef struct st_LWPSysFuncs {
    ...
    int                (*getBufData)(LWPSBufID buf,void        *data);  /* gets data from a buffer */
    ...
    int                (*getParticle)(LWPSBufID  buf,int idx,void  *data); /* gets a particle data from the given buffer */
    void               (*remParticle)(LWPSysID   ps,int idx);              /* removes a particle from the system */

    const LWItemID    *(*getPSysItems)(LWPSysID ps);                      /* returns null terminated list of items the PSys is attached to. LW2020*/
    const LWPSBufID   *(*getBufs)(LWPSysID      ps);                      /* returns null terminated list of buffers the PSys is using. LW2020*/
    const LWPSBufDesc *(*getDesc)(LWPSysID     ps, LWPSBufID buf);       /* returns null LWPSBufDesc for buffer. LW2020*/
    const LWPSBufID    (*getBufIDByName)(LWPSysID ps, const LWCStringASCII buf);  /* return buffer by name added by addBuf LW2020*/
} LWPSysFuncs;
