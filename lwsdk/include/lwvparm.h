...
#define LWVPARMFUNCS_GLOBAL  "LWVParmFuncs 4"
...
#define LWVP_COLORSIMPLE  0x61
#define LWVPF_VECTOR      0x01
#define LWVPF_VECTOR_DEFAULTNAMES  0x00
#define LWVPF_VECTOR_NONAMES       0x02
#define LWVPF_VECTOR_PARMNAMES     0x04
...
typedef enum en_LWVP_EVENTCODES
{
  ...
  LWVPEC_TEXOLD,
  LWVPEC_ENVRENAME,
  LWVPEC_TXGRADAUTOSIZE
} en_lwvpec;

typedef int LWVP_EventFunc( LWVParmID vp, void *userData, en_lwvpec eventCode, void *eventData );
 ...
 *              result is a 3 element array to be populated.
 ...
 *              3 doubles. If the parameter is enveloped, setting the value
 ...
 *              bit flags 'state' value to create or destroy the
 ...
 * editEnv:     If an envelope exists for the parameter, this will open
 *              the envelope editor.
 * editTex:     If a texture exists for the parameter, this will open
 *              the texture editor.
...
/* $wrapper: type=global, name=LWVParmFuncs$ */
typedef struct st_LWVParmFuncs
{
  ...
  void         (*setup)     ( LWVParmID parm,
                              LWCStringASCII channelName,
                              LWChanGroupID group,
                              LWTxtrContextID gc,
                              LWVP_EventFunc *eventFunc,
                              LWCStringUTF8 pluginName,
                              void *userData );
  ...
  double        (*getVal)   ( LWVParmID parm, LWTime t, LWMicropolID mp, double *result );
  ...
  /* $argument: method=LWVParmFuncs:getEnv, arg2=out:LWEnvelopeID:3$ */
