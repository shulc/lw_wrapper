...
#define LWHUMANINTERFACE_MANAGER_VERSION 2
...
// track access
typedef struct st_LWHumanInterfaceTrackAccess
{
    // create a track.  Storage of the track is up to the client-side device
    LWHumanInterfaceTrackID (*createTrack)( LWHumanInterfaceDeviceID in_device_id, LWDeviceTrackID in_track_id, LWCStringUTF8 in_display_name, int in_record_format, int in_event_driven);
...
} LWHumanInterfaceTrackAccess;

// device access
typedef struct st_LWHumanInterfaceDeviceAccess
{
...
} LWHumanInterfaceDeviceAccess;

// manager access
typedef struct st_LWHumanInterfaceManagerAccess
{
...
typedef struct st_LWHumanInterfaceDeviceCreateContext
{
...
} LWHumanInterfaceDeviceCreateContext;
...
typedef struct st_LWHumanInterfaceDeviceHandler
{
    ...
    LWCStringUTF8 (*signature)( LWInstance in_inst);
...
} LWHumanInterfaceDeviceHandler;

// a custom manager must supply these routines
typedef struct st_LWHumanInterfaceManagerHandler
{
    LWInstanceFuncs *inst;
...
} LWHumanInterfaceManagerHandler;
